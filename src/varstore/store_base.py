...
class VarStoreAppError(VarStoreError):
    """Base class for Application-level VarStore exceptions.

    These exceptions indicate errors in the application's configuration or setup.
    """


class AlreadyExistingSourceError(VarStoreAppError):
...
class ReferenceToMissingSourceError(VarStoreAppError):
...
class VarStoreUserError(VarStoreError):
    """Base class for User-level VarStore exceptions.

    These exceptions indicate errors in user input or usage.
    """


class InvalidVarNameError(VarStoreUserError):
...
class UndefinedVarError(VarStoreUserError):
...
class StoreManager:
    ...
    def set_scopes(self, *args: Union[Dict[str, List[str]], str, List[str]]) -> None:
        ...
        Raises:
            ...
            VarStoreAppError: If referenced sources don't exist or if there are circular references.
        """
